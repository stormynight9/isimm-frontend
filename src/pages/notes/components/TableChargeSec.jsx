    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                Header: `DS_${code}`,
                accessor: "noteDs",
                Cell: ({ row }) => <InputNote type="number" min="0" step="0.25" max="20" pattern="[0-20]" title="Please enter a number between 0 and 20." defaultValue={row.original.noteDs} onBlur={(event) => handleInputChange(event, row.index, "noteDs")} />,
            },
            {
                Header: `EX_${code}`,
                accessor: "noteExam",
                Cell: ({ row }) => <InputNote type="number" min="0" step="0.25" max="20" pattern="[0-20]" title="Please enter a number between 0 and 20." defaultValue={row.original.noteExam} onBlur={(event) => handleInputChange(event, row.index, "noteExam")} />,
            },
        ],
