...
export default function FormFacture({title, initialValues, type, id}) {
    ...
    function handleRecordsChange(records) {
        ...
        setValues(values => {
            ...
            let totalht = roundTo3(values.records.reduce((acc, record) => acc + (record.unit_price * record.quantity), 0));
            let totaltva = roundTo3(values.records.reduce((acc, record) => acc + ((record.unit_price * record.quantity) * (record.vat / 100)), 0));
            let total = roundTo3(values.records.reduce((acc, record) => acc + ((record.unit_price * record.quantity) * (1 + record.vat / 100)), 0));
    ...
    return <BaseForm title={title} onSave={handleSave} type={type}>
        <div className="flex flex-row my-2">
            <InputLabel className="flex-1 mr-1" disabled={type==='visit' ? true : undefined} name="date" label="Date" type="date" value={values.date} onChange={handleChange} />
            <Select className="flex-1 ml-1" disabled={type === 'add' ? undefined : true} items={transpileFournisseurs(fournisseurs)} accessor="id" name="fournisseur" label="Fournisseur" value={values.fournisseur} onChange={handleFournisseurChange} />
        </div>
        <div className="flex flex-row my-2">
            <InputLabel className="flex-1 mr-1" disabled={type==='visit' ? true : undefined} name="address" label="Adresse Facturation" value={values.address} onChange={handleChange} />
        </div>
        <InvoiceRecords products={products} records={values.records} onChange={handleRecordsChange} type={type}/>
        <div className="flex flex-col my-2">
            <InputLabel className="flex-1 mr-1" disabled label="Nombre de produits" type="number" value={values.nproducts || 0} onChange={() => {}} />
            <InputLabel className="flex-1 ml-1" disabled label="Quantité Totale" type="number" value={values.nitems || 0} onChange={() => {}} />
            <InputLabel className="flex-1 mr-1" disabled label="Total HT" type="number" value={values.totalht || 0} onChange={() => {}} />
            <InputLabel className="flex-1 ml-1" disabled label="TVA" type="number" value={values.totaltva || 0} onChange={() => {}} />
            <InputLabel className="flex-1 ml-1" disabled label="Total TTC" type="number" value={values.total || 0} onChange={() => {}} />
        </div>
    </BaseForm>
}