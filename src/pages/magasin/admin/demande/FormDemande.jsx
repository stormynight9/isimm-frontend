...
export default function FormDemande({
    title,
    type,
    employer,
    service,
    ...
    const [values, setValues] = useState(
        initialValues || { description: "", records: [{ id: 1, quantity: 1, product: 1 }] }
    )
    ...
    function showAccept() {
        return (
            type === "accepter" &&
            ((source === "employer" && values.status === "pending" && service) ||
                (source === "employer" && values.status === "approved" && employer) ||
                (source === "service" && values.status === "pending" && magasin) ||
                (source === "service" && values.status === "approved" && service))
        )
    }
    ...
    }, [isLoading, products, values.records])
    ...
    function handleSave() {
        fetchData(
            "POST",
            `${import.meta.env.VITE_API_URL}/api/isimm/gestionMagasin/magasin/DemandeStockable`,
            transpilePostDemandeBody({
        ...
        <BaseForm title={title} onSave={handleSave} type={type}>
            {values.status && (
                <div className="flex flex-row justify-end">
                    <DemandeBadge status={values.status} />
                </div>
            )}
            ...
            <DemandeRecords
                ...
                type={type}
                editQte={showQuantity()}
            />
            {/* <Input type="hidden" name="typeDemande" value={employer ? employer : 'service'} /> */}
            {showAccept() && (
                <div className="my-2 flex flex-row justify-end">
                    <Button variant="destructive" className="m-3" onClick={handleRefuser}>
                        Refuser
                    </Button>
                    <Button onClick={handleAccepter} className="m-3">
                        Accepter
                    </Button>
                </div>
            )}
